...
class UserKey:
   ...
   def konfirkeys(self):
       if os.path.isfile('data/.keys.txt') is True:
          try:
              ...
              self.cari = requests.get('https://pastebin.com/raw/eqWZpfeX').text
   ...
   def BuyLicen(self):
       ...
       try:
            self.req = requests.get('https://pastebin.com/raw/eqWZpfeX').text
       ...
       os.system(f'xdg-open https://wa.me/+6285658986563?text={self.xx}')
       open('data/.keys.txt','w').write(f'{self.dev}')
       open('data/.join.txt','w').write(f'{self.join}')
       sys.exit()

#-> Simpel Code By ArifXeyracode